...
int main(int argc, char *argv[]) {
    ...
    ofstream archivoSalida("MiResultadoSuma.txt");
    if (!archivoSalida.is_open()) {
        cerr << "Error al crear MiResultadoSuma.txt" << endl;
        return -1;
    }

    // --- Guardar la semilla ---
    archivoSalida << semilla << endl;

    // --- Guardar sumas ---
    for (int i = 0; i < cantidadPixeles * 3; i += 3) {
        ...
        archivoSalida << r << " " << g << " " << b << endl;
    }

    archivoSalida.close();
    cout << "\nArchivo MiResultadoSuma.txt creado exitosamente!" << endl;
